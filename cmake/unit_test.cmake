...
function(UNIT_TEST)
  ...
  set_target_properties(${TEST_NAME} PROPERTIES CXX_CLANG_TIDY "${WALBERLA_BRIDGE_CXX_CLANG_TIDY}")
  ...
  target_link_libraries(${TEST_NAME} PRIVATE walberla_bridge::config walberla_bridge::cpp_flags)

  # If NUM_PROC is given, set up MPI parallel test case
  if(TEST_NUM_PROC)
    if(${TEST_NUM_PROC} GREATER ${WALBERLA_BRIDGE_TEST_NP})
      set(TEST_NUM_PROC ${WALBERLA_BRIDGE_TEST_NP})
    endif()
    walberla_bridge_set_mpiexec_tmpdir(${TEST_NAME})
    add_test(${TEST_NAME} ${MPIEXEC} ${WALBERLA_BRIDGE_MPIEXEC_OVERSUBSCRIBE}
             ${MPIEXEC_NUMPROC_FLAG} ${TEST_NUM_PROC} ${MPIEXEC_PREFLAGS}
             ${WALBERLA_BRIDGE_MPIEXEC_TMPDIR} ${CMAKE_CURRENT_BINARY_DIR}/${TEST_NAME}
             ${MPIEXEC_POSTFLAGS})
  else()
    add_test(${TEST_NAME} ${TEST_NAME})
  endif()

  if(WALBERLA_BRIDGE_WARNINGS_ARE_ERRORS)
  ...
  if(NOT TEST_NUM_PROC AND WALBERLA_BRIDGE_MPIEXEC_GUARD_SINGLETON_NUMA AND "${TEST_DEPENDS}" MATCHES "(^|;)([Bb]oost::mpi|MPI::MPI_CXX)($|;)")
